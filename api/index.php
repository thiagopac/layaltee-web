...
function updateUserProfile(){
  ...
  if ($json->userLogin) { $sqlUserProfileUpdate .= "LOGIN = :userLogin, "; }
  if ($json->userPassword) { $sqlUserProfileUpdate .= "PASSWORD = MD5(:userPassword), "; }
  if ($json->userFirstName) { $sqlUserProfileUpdate .= "FIRSTNAME = :userFirstName, "; }
  if ($json->userLastName) { $sqlUserProfileUpdate .= "LASTNAME = :userLastName, ";}
  if ($json->userBirthday) { $sqlUserProfileUpdate .= "BIRTHDAY = :userBirthday, "; }
  if ($json->userEmail) { $sqlUserProfileUpdate .= "EMAIL = :userEmail, "; }
  if ($json->userGender) { $sqlUserProfileUpdate .= "GENDER = :userGender, "; }
  if ($json->userDeleted) { $sqlUserProfileUpdate .= "DELETED = :userDeleted, "; }
  if ($json->userUpdatedData) { $sqlUserProfileUpdate .= "UPDATED_DATA = :userUpdatedData, "; }
  if ($json->userPlatform) { $sqlUserProfileUpdate .= "PLATFORM = :userPlatform, "; }
  $sqlUserProfileUpdate .= "DELETED = 0 WHERE U.LOGIN = :userLogin AND U.LOCAL_ID = :userLocalId";
	...
	try{
      ...
      if ($userInfo == null) {
            ...
            if ($json->userBirthday) { $stmt->bindParam("userBirthday",$birth2); }
            ...
            //CONDITIONS TO MATCH
            if ($json->userLogin) { $stmt->bindParam("userLogin",$json->userLogin); }
            if ($json->userPassword) { $stmt->bindParam("userPassword",$json->userPassword); }
            if ($json->userFirstName) { $stmt->bindParam("userFirstName",$json->userFirstName); }
            if ($json->userLastName) { $stmt->bindParam("userLastName",$json->userLastName); }

            $birth = explode("/" ,$json->userBirthday);
            $birth2 = implode("-", array_reverse($birth));
            if ($json->userBirthday) { $stmt->bindParam("userBirthday",$birth2); }

            if ($json->userEmail) { $stmt->bindParam("userEmail",$json->userEmail); }
            if ($json->userGender) { $stmt->bindParam("userGender",$json->userGender); }
            if ($json->userDeleted) { $stmt->bindParam("userDeleted",$json->userDeleted); }
            if ($json->userUpdatedData) { $stmt->bindParam("userUpdatedData",$json->userUpdatedData); }
            if ($json->userLocalId) { $stmt->bindParam("userLocalId",$json->userLocalId); }

            $platform = $json->userPlatform;
            // echo json_encode($platform, JSON_NUMERIC_CHECK);
            // exit;

            if ($json->userPlatform) { $stmt->bindParam("userPlatform",$platform); }
  ...
  $sqlUserProfileUpdate .= "AND U.UPDATED_DATA = 0"; //só atualizar com os dados vindos do facebook se o usuário não os editou no aplicativo ainda
	...
	try{
      ...
      if ($userInfo == null) {
            ...
            if ($json->userLogin) { $stmt->bindParam("userLogin",$json->userLogin); }
            if ($json->userFirstName) { $stmt->bindParam("userFirstName",$json->userFirstName); }else{ $stmt->bindParam("userFirstName",$emptyParam); }
            if ($json->userLastName) { $stmt->bindParam("userLastName",$json->userLastName); }else{ $stmt->bindParam("userLastName",$emptyParam); }
            if ($json->userBirthday) { $stmt->bindParam("userBirthday",implode("-", array_reverse(explode("/", $json->userBirthday)))); }else{ $stmt->bindParam("userBirthday",$emptyParam); }
            if ($json->userEmail) { $stmt->bindParam("userEmail",$json->userEmail); }else{ $stmt->bindParam("userEmail",$emptyParam); }
            if ($json->userGender) { $stmt->bindParam("userGender",$json->userGender); }else{ $stmt->bindParam("userGender",$emptyParam); }
            if ($json->userLocalId) { $stmt->bindParam("userLocalId",$json->userLocalId); }
            if ($json->userPlatform) { $stmt->bindParam("userPlatform",$json->userPlatform); }
            $stmt->bindParam("userCode",$userCode);
            ...
            if ($json->userFirstName) { $stmt->bindParam("userFirstName",$json->userFirstName); }
            if ($json->userLastName) { $stmt->bindParam("userLastName",$json->userLastName); }
            if ($json->userBirthday) { $stmt->bindParam("userBirthday",implode("-", array_reverse(explode("/", $json->userBirthday)))); }
            if ($json->userEmail) { $stmt->bindParam("userEmail",$json->userEmail); }
            if ($json->userGender) { $stmt->bindParam("userGender",$json->userGender); }
            if ($json->userDeleted) { $stmt->bindParam("userDeleted",$json->userDeleted); }
            if ($json->userUpdatedData) { $stmt->bindParam("userUpdatedData",$json->userUpdatedData); }
